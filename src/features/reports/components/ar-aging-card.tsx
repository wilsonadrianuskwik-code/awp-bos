  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Outstanding balances as of today — not affected by the date range above.
      </p>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {buckets.map((bucket) => {
            // maxAmount is guaranteed > 0 here since totalOutstanding > 0
            // (at least one bucket is non-zero), but guard anyway so a
            // stale/zero maxAmount can never produce a NaN/Infinity width.
            const widthPercent =
              maxAmount > 0 ? (bucket.outstandingAmount / maxAmount) * 100 : 0;
