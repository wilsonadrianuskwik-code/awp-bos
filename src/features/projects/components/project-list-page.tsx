...
export function ProjectListPage({ projects, count }: ProjectListPageProps) {
  ...
  const selected = optimisticProjects.filter((p) => selectedIds.has(p.id));
  ...
  async function handleBulkDelete() {
    ...
    startTransition(async () => {
      removeOptimisticProjects(idsToDelete);
      const results = await Promise.all(
        toDelete.map((p) => deleteProject(workspace.id, p.id))
      );
  ...
  return (
    <div className="space-y-4">
      ...
      {optimisticProjects.length === 0 ? (
        <ListEmpty message="No projects match your filters." />
      ) : (
        <DataTable
          columns={columns}
          ...
          selection={
            can("staff")
              ? { selectedIds, onSelectedIdsChange: setSelectedIds, getId: (p) => p.id }
              : undefined
          }
        />
      )}
