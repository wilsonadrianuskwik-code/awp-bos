...
import { regenerateInvoiceShareToken } from "@/features/invoices/actions";
...
export function InvoicePortalAccessCard({
  ...
  function copyLink() {
    const url = `${window.location.origin}/portal/invoices/${shareToken}`;
  ...
  function handleRegenerate() {
    startTransition(async () => {
      const result = await regenerateInvoiceShareToken(workspace.id, invoice.id);
      if (result.error) {
          ...
          {invoice.view_count > 0 ? (
            <>
              <p className="font-medium">
                Viewed {invoice.view_count} time
                {invoice.view_count === 1 ? "" : "s"}
              </p>
              {invoice.last_viewed_at && (
                <p className="text-xs text-muted-foreground">
                  Last viewed: {formatTimelineTimestamp(invoice.last_viewed_at)}
                </p>
              )}
            </>
          ) : (
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              sent — will no longer be able to open this invoice.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
